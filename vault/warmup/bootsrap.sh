#!/usr/bin/env bash
    ...
    VAULT_PODS_IN_STATEFULSET=$(expr $(kubectl get -n $NAMESPACE statefulsets -o json | jq '.items[0].spec.replicas') - 1)

    # Setup all Vault pods in an array
    VAULT_PODS=($(seq --format='prod-vault-%0g' --separator=" " 0 $VAULT_PODS_IN_STATEFULSET))

    # Raft leaders and followers
    VAULT_LEADER=${VAULT_PODS[0]}
    echo -e "Vault Leader: $VAULT_LEADER"
    VAULT_FOLLOWERS=("${VAULT_PODS[@]:1}")

    # Wait for pod to be ready
    function waitForPod {
      while [[ $(kubectl get -n $NAMESPACE pods $1 -o 'jsonpath={..status.conditions[?(@.type=="Ready")].status}') != "True" ]] ; do
    ...
    function vaultOperatorInit {
      #waitForPod $1
      kubectl exec -n $NAMESPACE $1 -c vault -- vault operator init -format "json" > $VAULT_INIT_OUTPUT_FILE
    }
...
type: Opaque
data:
  VAULT_INIT_JSON: $VAULT_INIT_JSON_BASE64
EOF
...
function unsealVault {
  local VAULT_INIT_JSON_BASE64_DECODED=$(kubectl get -n $NAMESPACE secrets/vault-init --template={{.data.VAULT_INIT_JSON}} | base64 -d)
  for VAULT_UNSEAL_KEY in $(jq -r '.unseal_keys_b64[]' <<< ${VAULT_INIT_JSON_BASE64_DECODED}) ; do
    # waitForPod $1
    echo -e "Unsealing Vault on pod $1"
    sleep 5
    kubectl exec -i -n $NAMESPACE $1 -- vault operator unseal $VAULT_UNSEAL_KEY
  done
}

    function joinRaftLeader() {
      # waitForPod $1
      kubectl exec -n $NAMESPACE $1 -- vault operator raft join http://$VAULT_LEADER.vault-internal:8200

    }

    # Get Vault initialization status
    # waitForPod $VAULT_LEADER
    VAULT_INIT_STATUS=$(kubectl exec -n $NAMESPACE $VAULT_LEADER -c vault -- vault status -format "json" | jq --raw-output '.initialized')
    ...
    if $VAULT_INIT_STATUS ; then
      VAULT_SEAL_STATUS=$(kubectl exec -n $NAMESPACE $VAULT_LEADER -c vault -- vault status -format "json" | jq --raw-output '.sealed')
    ...
    for POD in "${VAULT_FOLLOWERS[@]}" ; do
      VAULT_TOKEN=$(kubectl get -n $NAMESPACE secrets/vault-init --template={{.data.VAULT_INIT_JSON}} | base64 -d | jq -r '.root_token')
      RAFT_NODES_JSON=$(kubectl exec -n $NAMESPACE $VAULT_LEADER -c vault -- /bin/sh -c "VAULT_TOKEN=$VAULT_TOKEN vault operator raft list-peers -format \"json\"")
      RAFT_NODES=$(echo $RAFT_NODES_JSON | jq '.data.config.servers[].address' -r)
      #waitForPod $POD
      VAULT_SEAL_STATUS=$(kubectl exec -n $NAMESPACE $POD -c vault -- vault status -format "json" | jq --raw-output '.sealed')
