...
class Client(object):
  ...
  def _load_config(self, host, client_id, namespace, other_client_id, other_client_secret, existing_token, proxy, ssl_ca_cert, kube_context, credentials):
    ...
    if in_cluster:
      ...
      # If no credentials provided, attempt to set up credentials assuming a
      # projected volume
      ...
      try:
        ...
        logging.warning("Failed to initialize projected service account token"
                        " credentials. Proceeding without them...",
                        exc_info=True)
      else:
        config.refresh_api_key_hook = credentials.refresh_api_key_hook
        config.api_key['authorization'] = token
