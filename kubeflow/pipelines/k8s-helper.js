 ...
 */
function parseTensorboardLogDir(logdir, podTemplateSpec) {
    var e_1, _a;
    var urls = [];
    var seriesParts = logdir.split(',');
    try {
        for (var seriesParts_1 = __values(seriesParts), seriesParts_1_1 = seriesParts_1.next(); !seriesParts_1_1.done; seriesParts_1_1 = seriesParts_1.next()) {
            var seriesPart = seriesParts_1_1.value;
            var strPath = seriesPart.replace(/Series\d+:/g, '');
            if (!strPath.startsWith('volume://')) {
                urls.push(strPath);
                continue;
            }
            // volume storage: parse real local mount path
            var pathParts = strPath.substr('volume://'.length).split('/');
            var bucket = pathParts[0];
            var key = pathParts.slice(1).join('/');
            var _b = __read(utils_1.findFileOnPodVolume(podTemplateSpec, {
                volumeMountName: bucket,
                filePathInVolume: key,
                containerNames: undefined,
            }), 2), filePath = _b[0], err = _b[1];
            if (err) {
                throw new Error(err);
            }
            urls.push(filePath);
        ...
        try {
            if (seriesParts_1_1 && !seriesParts_1_1.done && (_a = seriesParts_1.return)) _a.call(seriesParts_1);
        }
        finally { if (e_1) throw e_1.error; }
    }
    return urls.length === 1 ? urls[0] : urls.map(function (c, i) { return "Series" + (i + 1) + ":" + c; }).join(',');
}
/**
...
function newTensorboardInstance(logdir, namespace, tfImageName, tfversion, podTemplateSpec) {
    ...
    return __awaiter(this, void 0, void 0, function () {
        var currentPod, tensorboardImage, body;
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    tensorboardImage = tfImageName + (tfversion ? ":" + tfversion : '');
                    body = {
                ...
                case 0: return [4 /*yield*/, k8sV1CustomObjectClient
                    ...
                    function (viewer) {
                        if (viewer && viewer.body && viewer.body.spec.type === 'tensorboard') {
                            ...
                            var image = viewer.body.spec.tensorboardSpec.tensorflowImage;
                            var tfImageParts = image.split(':', 2);
                            var tfVersion = tfImageParts.length == 2 ? tfImageParts[1] : '';
                            return { podAddress: address, tfVersion: tfVersion, image: image };
                        }
                        else {
...
function getPodLogs(podName, podNamespace, containerName) {
    if (containerName === void 0) { containerName = 'main'; }
    ...
    return k8sV1Client.readNamespacedPodLog(podName, podNamespace, containerName).then(function (response) { return (response && response.body ? response.body.toString() : ''); }, function (error) {
 ...
 */
function getConfigMap(configMapName, configMapNamespace) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 2, , 4]);
                    return [4 /*yield*/, k8sV1Client.readNamespacedConfigMap(configMapName, configMapNamespace)];
                case 1:
                    body = (_b.sent()).body;
                    return [2 /*return*/, [body, undefined]];
                case 2:
                ...
                case 3:
                    _a = _b.sent(), message = _a.message, additionalInfo = _a.additionalInfo;
                    userMessage = "Could not get configMap " + configMapName + " in namespace " + configMapNamespace + ": " + message;
                    return [2 /*return*/, [undefined, { message: userMessage, additionalInfo: additionalInfo }]];
                case 4: return [2 /*return*/];
            }
        });
...
function listPodEvents(podName, podNamespace) {
    return __awaiter(this, void 0, void 0, function () {
        var body, error_3, _a, message, additionalInfo, userMessage;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 2, , 4]);
                ...
                case 2:
                    error_3 = _b.sent();
                    return [4 /*yield*/, utils_1.parseError(error_3)];
                case 3:
                    _a = _b.sent(), message = _a.message, additionalInfo = _a.additionalInfo;
...
async function readSecret(name, namespace) {
    try {
      const response = await k8sV1Client.readNamespacedSecret(name, namespace);
      console.log('Secret data:', response.body.data);
    } catch (error) {
      console.error('Error reading secret:', error.response ? error.response.body : error.message);
    }
  }
  
function getK8sSecret(name, key, providedNamespace) {
    return __awaiter(this, void 0, void 0, function () {
        var namespace, k8sSecret, secretb64, buff;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    console.log('je suis là avant le call');
                    ...
                    readSecret(name, namespace)
                    if (!namespace) {
                        throw new Error("Cannot get namespace from " + namespaceFilePath);
                    }
                    return [4 /*yield*/, k8sV1Client.readNamespacedSecret(name, namespace)];
                case 1:
                    k8sSecret = _a.sent();
                    secretb64 = k8sSecret.body.data[key];
                    console.log("je suis ici")
                    console.log("secretb64 value: ", secretb64)
                    /** console.log("secretb64 value: ", secretb64) **/
...
exports.TEST_ONLY = {
    ...
    parseTensorboardLogDir: parseTensorboardLogDir,
};